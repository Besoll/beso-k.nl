...
export default function FormSMTP() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await sendMailAction(formValues);
      setIsSubmitted(true);
    } catch (error) {
      ...
      {isSubmitted ? (
        ...
        >
          ...
          {Object.entries(formValues).map(([name, value], index) => (
            <div
              key={name}
              className={`w-full ${index === Object.entries(formValues).length - 1 ? 'col-span-1 md:col-span-2' : 'col-span-2 md:col-span-1'}`}
            >
              {index === Object.entries(formValues).length - 1 ? (
                <textarea
                  ...
                  rows={4}
                  className={`bg-white shadow-md rounded-xl w-full border-none px-4 mb-2 mt-2 py-4 md:py-4 ${formErrors[name] ? 'border-red' : 'file-input-bordered file-input-success'}`}
